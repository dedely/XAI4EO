...
def evaluation(model, data_loader, device, class_names, criterion=None, mode='val', temporal_shift=None):
    ...
    metrics = {
        ...
        'classification_dict': sklearn.metrics.classification_report(y_true, y_pred, labels=list(range(len(class_names))), target_names=class_names, zero_division=0, output_dict=True),
