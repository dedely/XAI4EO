...
from utils import label_utils_notebook
...
class PixelSetData(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.kept_idx is not None:
            if self.occluded_class is not None :
                ...
                zero_idx = np.where(self.kept_idx[y] == 0)
                pixels[zero_idx, :, :] = 0
                #pixels[zero_idx] = np.zeros((pixels.shape[1], pixels.shape[2]))
    ...
    def make_dataset(self, data_folder, meta_folder, class_to_idx, indices, country):
        ...
        instances = []
        code_to_class_name = label_utils_notebook.get_code_to_class(country)
    ...
    def get_unknown_labels(self):
        ...
        code_to_class_name = label_utils_notebook.get_code_to_class(self.country)
        codification_table = label_utils_notebook.get_codification_table(self.country)
        unknown = []
...
class PixelSetTensorData(PixelSetData):
    def __getitem__(self, index):
        path, date_positions, n_pixels, y, extra, gdd = self.samples[index]
        pixels = zarr.load(path)  # (T, C, S)
        if self.kept_idx is not None:
        ...
        # extra = sample['extra']
        # gdd = sample['gdd']
        input_args = (pixels, valid_pixels, positions, extra, gdd)
        target = sample['label']
        target = target.cuda(device=device, non_blocking=True)
        # print(type(target))
        # print(type(input_args))
        # As per captum hidden implementation, data needs to be formated as a tuple of (inputs, labels)
        return (input_args, target)
